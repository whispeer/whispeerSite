...
function SecureRandom() {
    function nextBytes(byteArray) {
        ...
        for (n = 0; n < byteArray.length; n = n + 1) {
            byteArray[n] = sjcl.random.randomWords(1, ssn.config.paranoia)[0];
        }
    }
	...
	*/
	...
	function pssVerify(mHash, EM, emBits) {
		...
		if (emLen < hlen + slen + 2) {
			ssn.logger.log("emLen wrong");
			return false;
		}
		
		if (EM.substr(EM.length - 2) !== "bc") {
			ssn.logger.log("not bc");
			return false;
		}
	...
	*/
	function generateAsync(B, E, callback) {
		...
		primeCalculator.onmessage = function (event) {
			ssn.logger.log("data from worker!");
...
/*						if (key.d.toString(16).length != B / 4 || key.n.toString(16).length != B / 4) {
					...
					ssn.logger.logError("Worker Error! Worker produced more primes than needed");
				}
			}
