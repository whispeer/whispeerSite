...
function () {
    ...
    sjcl.bitArray = {
        ...
        concat: function (c, a) {
            ...
            var d, e, f = c[c.length - 1],
                b = sjcl.bitArray.getPartial(f);
            if (b === 32) {
                return c.concat(a)
            } else {
    ...
    sjcl.hash.sha256.prototype = {
        ...
        update: function (f) {
            ...
            var e, a = this._buffer = sjcl.bitArray.concat(this._buffer, f),
                d = this._length,
                c = this._length = d + sjcl.bitArray.bitLength(f);
    ...
    sjcl.random = {
        ...
        addEntropy: function (e, l, a) {
            ...
            switch (typeof (e)) {
                case "number":
                    ...
                    break;
                case "object":
                    var h = Object.prototype.toString.call(e);
        ...
        _mouseCollector: function (b) {
            var a = b.x || b.clientX || b.offsetX || 0,
                c = b.y || b.clientY || b.offsetY || 0;
    ...
    sjcl.json = {
        ...
        decrypt: function (i, b, c, f) {
            ...
            f = f || {};
            var d = sjcl.json,
                a = d._add(d._add(d._add({}, d.defaults), d.decode(b)), c, true),
                g, e, h, k = a.adata;
            if (typeof a.salt === "string") {
                a.salt = sjcl.codec.base64.toBits(a.salt)
            }
    ...
    sjcl.bn.pseudoMersennePrime = function (f, b) {
        ...
        a.reduce = function () {
            var q, p, o, n = this.modOffset,
                t = this.limbs,
                j, m = this.offset,
                r = this.offset.length,
                h = this.factor,
                s;
            q = this.minOffset;
            while (t.length > n) {
    ...
    sjcl.bn.prime = {
        ...
        p256: sjcl.bn.pseudoMersennePrime(0x100, [
            [0, -1],
            [96, 1],
            [192, 1],
            [224, -1]
    ...
    sjcl.ecc.pointJac.prototype = {
        ...
        doubl: function () {
            ...
            var g = this.y.square(),
                ...
                e = g.square().mul(8),
                h = this.z.square(),
                k = this.x.sub(h).mul(3).mul(this.x.add(h)),
    ...
    sjcl.ecc._dh = function (a) {
        sjcl.ecc[a] = {
            publicKey: function (c, b) {
                this._curve = c;
                if (b instanceof Array) {
                    this._point = c.fromBits(b)
                } else {
                    this._point = b
                }
            },
            secretKey: function (c, b) {
                this._curve = c;
                this._exponent = b
            },
			//e: curve
			//d: entropy needed
            generateKeys: function (e, d) {
                if (e === undefined) {
                    e = 0x100
                }
                if (typeof e === "number") {
                    e = sjcl.ecc.curves["c" + e];
                    if (e === undefined) {
                        throw new sjcl.exception.invalid("no such curve")
                    }
                }
                var b = sjcl.bn.random(e.r, d);
                var c = e.G.mult(b);

                return {
                    pub: new sjcl.ecc[a].publicKey(e, c),
                    sec: new sjcl.ecc[a].secretKey(e, b)
                }
            }
        }
    };
    ...
    sjcl.ecc.ecdsa.secretKey.prototype = {
        sign: function (g, f) {
            var d = this._curve.r,
                a = d.bitLength(),
                b = sjcl.bn.random(d.sub(1), f).add(1),
                e = this._curve.G.mult(b).x.mod(d),
                c = sjcl.bn.fromBits(g).add(e.mul(this._exponent)).inverseMod(d).mul(b).mod(d);
    ...
    sjcl.ecc.ecdsa.publicKey.prototype = {
        verify: function (e, d) {
            var h = sjcl.bitArray,
                f = this._curve.r,
                c = f.bitLength(),
                a = sjcl.bn.fromBits(h.bitSlice(d, 0, c)),
                j = sjcl.bn.fromBits(h.bitSlice(d, c, 2 * c)),
                g = sjcl.bn.fromBits(e).mul(j).mod(f),
                i = a.mul(j).mod(f),
                b = this._curve.G.mult2(g, i, this._point).x;
