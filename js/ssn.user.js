"use strict";
if (typeof (ssn) === "undefined") {
	var ssn = {};
}

//short redefine.
var ssnH = ssn.helper;

//TODO: Add more attributes.
//TODO: Update own profile.

/** manages user. */
ssn.userManager = function () {
	/** valid profile attributes */
	this.validAttributes = {"firstname": true, "lastname": true};
	/** users by id */
	var usersID = {};
	/** users by nickname */
	var usersNickname = {};
	/** user loaded time */
	var loadedTime = {};

	/** own friends */
	var friends;
	/** own friendShipRequests */
	var friendShipRequests;
	/** own friend ship requested from other persons */
	var friendShipRequested;
	/** when were friends loaded? */
	var friendsLoaded = 0;
	/** what kind of keys do user have? */
	var userKeys = ["wall", "profile", "share"];

	this.reset = function () {
		usersID = {};
		usersNickname = {};
		loadedTime = {};

		friendsLoaded = 0;
	};

	var User = function (data) {
		/** users friends list */
		var friendsList;

		/** userid */
		var userid = 0;
		
		/** users nickname */ 
		var nickname;

		/** users public profile */
		var publicProfile;
		/** users private profiles */
		var profiles;

		/** scheme for user profiles. defines how much of user profiles was already loaded */
		var scheme = 0;

		/** users public key */
		var publicKey;

		/** users session keys */
		var sessionKeys = {};

		/** to have this in other contextes. */
		var that = this;

		/** transform a keys json object to a keys object
		* @param k keys json representation
		* @author Nilos
		*/
		var keysToObjects = function (k) {
			ssn.logger.log("keysToObject");

			var i = 0, currentKey = null;
			for (i = 0; i < userKeys.length; i += 1) {
				var uK = userKeys[i];
				if (ssnH.arraySet(k, uK)) {
					if (uK === "profile" && that.ownUser()) {
						var groupid;
						for (groupid in k[uK]) {
							if (k[uK].hasOwnProperty(groupid)) {
								currentKey = k[uK][groupid];
								if (!(currentKey instanceof ssn.crypto.sessionKey)) {
									currentKey = new ssn.crypto.sessionKey(currentKey);

									if (currentKey.isSymKey()) {
										currentKey.decryptKey(ssn.session.mainKey);
									} else {
										if (currentKey.isSymKey() === false) {
											ssnH.setSymAsymKey(currentKey);
										}
									}

									k[uK][groupid] = currentKey;
								}
							}
						}
					} else {
						if (k[uK] === false) {
							delete k[uK];
						} else {
							currentKey = k[uK];
							if (!(currentKey instanceof ssn.crypto.sessionKey)) {
								currentKey = new ssn.crypto.sessionKey(currentKey);

								if (currentKey.isSymKey()) {
									currentKey.decryptKey(ssn.session.mainKey);
								} else {
									if (currentKey.isSymKey() === false) {
										ssnH.setSymAsymKey(currentKey);
									}
								}

								k[uK] = currentKey;
							}
						}
					}
				} else {
					ssn.logger.log("Session Key problem. Sessionkey for '" + uK + "' not found");
					ssn.logger.log(k);
				}
			}

			return k;
		};

		/** get the link for the user */
		this.getLink = function () {
			return "#view=profile&userid=" + this.getUserID();
		};

		/** get the scheme */
		this.getScheme = function () {
			return scheme;
		};

		var uLoadProfile = function (profileData) {
			//this needs some improvements, to not re-decrypt data.
			profiles = profileData;

			var value;
			if (that.ownUser()) {
				var groupid;
				for (groupid in profiles) {
					if (profiles.hasOwnProperty(groupid)) {
						for (value in profiles[groupid]) {
							if (profiles[groupid].hasOwnProperty(value) && value !== "iv") {
								profiles[groupid][value] = {d: false, v: profiles[groupid][value]};
							}
						}
					}
				}
			} else {
				for (value in profiles) {
					if (profiles.hasOwnProperty(value) && value !== "iv") {
						profiles[value] = {d: false, v: profiles[value]};
					}
				}
			}
		};

		/** load profile from json
		* @param profile json representation of profile
		* @author Nilos
		*/
		this.loadFromJSON = function (profile) {
			var jsonData;
			if (typeof profile === "string") {
				jsonData = jQuery.parseJSON(profile);
			} else {
				jsonData = profile;
			}

			if (jsonData === null) {
				return false;
			}

			if (typeof jsonData.id !== "undefined") {
				if (userid === jsonData.id || userid === 0) {
					userid = jsonData.id;
					nickname = jsonData.nickname;

					scheme = jsonData.scheme;

					if (ssnH.isset(jsonData.sessionKey)) {
						sessionKeys = jsonData.sessionKey;
						sessionKeys = keysToObjects(sessionKeys);
					}

					publicProfile = jsonData["public"];
					uLoadProfile(jsonData["private"]);

					return true;
				}
			}

			return false;
		};

		/** is this the own user? */
		this.ownUser = function () {
			return (parseInt(ssn.session.userid, 10) === parseInt(userid, 10));
		};

		/** get this users name
		* returns: firstname lastname or nickname if no firstname and lastname set.
		*/
		this.getName = function () {
			if (this.getValue("firstName") !== "" || this.getValue("lastName") !== "") {
				return this.getValue("firstName") + " " + this.getValue("lastName");
			}

			if (typeof nickname !== "undefined" && nickname !== "") {
				return nickname;
			}

			return ssn.translation.getValue("unknownName");
		};

		this.getValue = function (name, group) {
			name = name.toLowerCase();
			//ownUser?
			if (this.ownUser()) {
				if (typeof group !== "number") {
					var groupid;
					for (groupid in profiles) {
						if (profiles.hasOwnProperty(groupid)) {
							group = groupid;
							break;
						}
					}
				}

				if (ssnH.arraySet(profiles, group, name) && ssnH.arraySet(sessionKeys, "profile")) {
					if (profiles[group][name].d === false) {
						profiles[group][name].v = ssn.crypto.decryptText(ssn.session.key,
							'{"ct": "' + profiles[group][name].v + '", "iv": "' + profiles[group].iv + '"}',
							sessionKeys.profile[group]);
						profiles[group][name].d = true;
					}

					return profiles[group][name].v;
				}
			//not own user
			} else {
				if (ssn.helper.arraySet(profiles, name)) {
					if (profiles[name].d === false) {
						profiles[name].v = ssn.crypto.decryptText(ssn.session.key,
							'{"ct": "' + profiles[name].v + '", "iv": "' + profiles.iv + '"}', sessionKeys.profile);
						profiles[name].d = true;
					}

					return profiles[name].v;
				}
			}

			if (ssn.helper.arraySet(publicProfile, name)) {
				return publicProfile[name];
			}

			return "";
		};

		/** get this users id */
		this.getUserID = function () {
			return userid;
		};

		/** get this users nickname */
		this.getNickname = function () {
			return nickname;
		};

		/** get this users public key
		* @param callback called with the public key
		* @param overwrite load key from server if true
		* @author Nilos
		* This needs a callback because the key is not automatically send with the profile.
		*/
		this.getPublicKey = function (callback, overwrite) {
			if (!ssnH.isset(publicKey) || overwrite === true) {
				ssn.helper.getData({"publicKey": [userid]}, function (data) {
					publicKey = new ssn.crypto.publicKey(data.publicKey[userid]);
					callback(publicKey);
				});
			} else {
				callback(publicKey);
			}
		};

		/** check a signature for this user.
		* @param signature the signature
		* @param message original message
		* @param callback called when result is ready
		* @author Nilos
		* gets public key and checks signature.
		*/
		this.checkSignature = function (signature, message, callback) {
			this.getPublicKey(function (p) {
				ssn.crypto.verifyText(p, message, signature, callback);
			});
		};

		/** get this users session keys
		* @param callback called with the session keys
		* @param overwrite load keys from server if true
		* @author Nilos
		*/
		this.getSessionKeys = function (callback, overwrite) {
			if (typeof sessionKeys === "undefined" || overwrite === true) {
				var getData = {"sessionKey": {}};
				var i = 0;
				for (i = 0; i < userKeys.length; i += 1) {
					getData.sessionKey[userKeys[i]] = [userid];
				}

				var ownUser = this.ownUser();

				ssn.helper.getData(getData, function (data) {
					sessionKeys = keysToObjects(data.sessionKey, ownUser);
					callback(sessionKeys);
				});
			} else {
				callback(sessionKeys);
			}
		};

		/** decrypt this users key
		* @param callback called when keys are decrypted
		* @author Nilos
		*/
		this.decryptKeys = function (callback) {
			var k = sessionKeys;

			var i = 0;
			for (i = 0; i < userKeys.length; i += 1) {
				var uK = userKeys[i];
				if (ssnH.arraySet(k, uK)) {
					if (uK === "profile" && this.ownUser()) {
						var groupid;
						for (groupid in k[uK]) {
							if (k[uK].hasOwnProperty(groupid)) {
								if (k[uK][groupid] instanceof ssn.crypto.sessionKey) {
									k[uK][groupid].decryptKey(ssn.session.key);
								}
							}
						}
					} else {
						if (k[uK] instanceof ssn.crypto.sessionKey) {
							k[uK].decryptKey(ssn.session.key);
						}
					}
				} else {
					ssn.logger.log("Session Key problem. Sessionkey for '" + uK + "' not found");
					ssn.logger.log(k);
				}
			}

			return;
		};

		/** get this users friends as object
		* @param callback called with user list
		* @author Nilos
		*/
		this.friends = function (callback) {
			ssn.helper.getData({"friends": userid}, function (f) {
				ssn.userManager.loadUsers(f.friends, ssn.userManager.BASIC, function (u) {
					friendsList = u;
					callback(u);
				});
			});
		};

		/** check if this user has a certain friend
		* @param userid userid of friend to check for
		* @param callback called with result
		* @author Nilos
		*/
		this.hasFriend = function (userid, callback) {
			ssn.helper.getData({"friends": userid}, function (f) {
				var friends = f.friends;
				var i, done = false;
				for (i = 0; i < friends.length; i += 1) {
					if (friends[i] === userid) {
						callback(true);
						done = true;
					}
				}

				if (!done) {
					callback(false);
				}
			});
		};

		/** checks if this user is a friend */
		this.isFriend = function () {
			var friends = ssn.userManager.friends();
			var i;
			for (i = 0; i < friends.length; i += 1) {
				if (friends[i] === this.getUserID()) {
					return true;
				}
			}

			return false;
		};

		/** checks if this user has requested a friendship */
		this.hasFriendShipRequested = function () {
			var friendShipRequests = ssn.userManager.friendShipRequests();
			var i;
			for (i = 0; i < friendShipRequests.length; i += 1) {
				if (friendShipRequests[i] === this.getUserID()) {
					return true;
				}
			}

			return false;
		};

		/** checks if own user has requested friendship */
		this.didIRequestFriendShip = function () {
			var friendShipRequested = ssn.userManager.friendShipRequested();
			var i;
			for (i = 0; i < friendShipRequested.length; i += 1) {
				if (friendShipRequested[i] === this.getUserID()) {
					return true;
				}
			}

			return false;
		};

		/** friendship this user
		* @param callback called with true if success
		* @group group to put this user into
		*/
		this.friendShip = function (callback, group) {
			if (typeof group === "undefined") {
				group = 1;
			}

			var time = new Date().getTime();

			this.getPublicKey(function (publicKey) {
				ssn.logger.log("pubKey:" + ((new Date().getTime()) - time));
				ssn.session.getOwnUser(function (u) {
					ssn.logger.log("ownUser:" + ((new Date().getTime()) - time));
					u.getSessionKeys(function (keys) {
						ssn.logger.log("skeys:" + ((new Date().getTime()) - time));
						ssn.crypto.signText(ssn.session.key, "friendShip" + userid, function (signature) {
							ssn.logger.log("sign:" + ((new Date().getTime()) - time));
							ssn.crypto.waitForReady(function () {
								ssn.logger.log("ready:" + ((new Date().getTime()) - time));

								var resultKeys = {};

								var i = 0;
								for (i = 0; i < userKeys.length; i += 1) {
									if (userKeys[i] === "profile") {
										resultKeys[userKeys[i]] = ssn.crypto.encryptSessionKey(publicKey, keys[userKeys[i]][group], ssn.session.key);
									} else {
										resultKeys[userKeys[i]] = ssn.crypto.encryptSessionKey(publicKey, keys[userKeys[i]], ssn.session.key);
									}
								}

								ssn.logger.log("keys encrypted:" + ((new Date().getTime()) - time));

								var friendShip = [];
								var friendShipR = {
									"id": userid,
									"sig": signature,
									"keys": resultKeys
								};

								friendShip.push(friendShipR);
								var getData = {
									"friendShip": friendShip
								};

								ssn.logger.log("sending:" + ((new Date().getTime()) - time));

								ssn.helper.getData(getData, function (data) {
									ssn.userManager.loadFriends(function () {
										if (data.friendShip[userid] === "true" || data.friendShip[userid]) {
											ssn.logger.log("done:" + ((new Date().getTime()) - time));
											callback(true);
										} else {
											callback(false);
										}
									}, true);
								});
							});
						});
					});
				});
			});
		};

		//load data from json
		if (!this.loadFromJSON(data)) {
			ssn.logger.log("User loading failed!");
			ssn.logger.log(data);

			throw new ssn.exception.userNotExisting();
		}
	};

	/** Get an already loaded user from arrays
	* @param identifier identifier of the user.
	* @author Nilos
	*/
	var umGetLoadedUser = function (identifier) {
		if (ssn.helper.isID(identifier)) {
			return usersID[identifier];
		}

		if (ssn.helper.isNickname(identifier)) {
			return usersNickname[identifier];
		}

		return false;
	};

	/** Add a user to the loaded lists. Update timestamp.
	* @param user user object to add to cache objects.
	*/
	var umAddLists = function (user) {
		var id = user.getUserID();
		var nickname = user.getNickname();

		usersID[id] = user;
		loadedTime[id] = new Date().getTime();

		if (typeof nickname !== "undefined") {
			usersNickname[nickname] = user;
		}
	};

	/** make raw userdata to users */
	var umMakeUsers = function (profiles) {
		var id;
		var users = [];
		for (id in profiles) {
			if (profiles.hasOwnProperty(id)) {
				if (profiles[id] !== "false") {
					var theUser;

					if (ssn.userManager.userLoaded(id)) {
						theUser = usersID[id];
						theUser.loadFromJSON(profiles[id]);
					} else {
						theUser = new User(profiles[id]);
					}

					umAddLists(theUser);
					users.push(theUser);
				}
			}
		}

		return users;
	};

	/** Load users.
	* identifiers: array of identifiers to load users for.
	* scheme: how many data to load. "scheme"
	* callback: function to call onload. called with array of users.
	*/
	var umLoadUsers = function (identifiers, scheme, callback) {
		var i = 0;
		var userRequest = {};
		var usersLoaded = [];

		if (identifiers.length === 0) {
			callback([]);
			return;
		}

		var request = false;

		for (i = 0; i < identifiers.length; i += 1) {
			var theUser = umGetLoadedUser(identifiers[i]);
			if (this.userLoaded(identifiers[i]) && theUser.getScheme() >= scheme) {
				usersLoaded.push(theUser);
			} else {
				userRequest[identifiers[i]] = scheme;
				request = true;
			}
		}

		if (request) {
			var getData = {"userProfile": userRequest};
			var that = this;

			ssn.helper.getData(getData, function (data) {
				try {
					if (typeof data.userProfile === "object") {
						var users = umMakeUsers(data.userProfile);
						callback(users.concat(usersLoaded));
					}
				} catch (e) {
					ssn.logger.log(e);
					throw e;
				}
			});
		} else {
			callback(usersLoaded);
		}
	};

	/** Load one user.
	* identifier: identifier of user.
	* scheme: "scheme"
	* callback: function to call when user is loaded
	**/
	var umGetUser = function (identifier, scheme, callback) {
		this.loadUsers([identifier], scheme, function (users) {
			callback(users[0]);
		});
	};

	var umUserLoaded = function (identifier) {
		if (ssn.helper.isID(identifier)) {
			return typeof usersID[identifier] === "object" && usersID[identifier] instanceof User;
		}

		if (ssn.helper.isNickname(identifier)) {
			return typeof usersNickname[identifier] === "object" && usersNickname[identifier] instanceof User;
		}

		//not a valid user identifier
		throw new ssn.exception.invalidUserIdentifier(identifier);
	};

	var umSearch = function (search, callback) {
		ssn.helper.getData({"search": search}, function (data) {
			var results = [];

			var theUser;
			var user;
			for (user in data.search) {
				if (data.search.hasOwnProperty(user)) {
					if (ssn.userManager.userLoaded(user)) {
						theUser = umGetLoadedUser(user);
					} else {
						theUser = new User(data.search[user]);
					}

					results.push(theUser);
				}
			}

			callback(results);
		});
	};

	var umGetPublicKeys = function (userids, callback) {
		ssn.helper.getData({"publicKey": userids}, function (data) {
			var key;
			var keys = [];

			for (key in data.publicKey) {
				if (data.publicKey.hasOwnProperty(key)) {
					keys.push(new ssn.crypto.publicKey(data.publicKey[key]));
				}
			}

			callback(keys);
		});
	};

	var umLoadFriends = function (callback, loadAnyHow) {
		if (loadAnyHow === true || ssn.config.reloadAfter < new Date().getTime() - friendsLoaded) {
			ssn.helper.getData({
				"friendShipRequests": "true",
				"friendShipRequested": "true",
				"friends": "true"
			}, function (data) {
				friends = data.friends;
				friendShipRequests = data.friendShipRequests;
				friendShipRequested = data.friendShipRequested;

				friendsLoaded = new Date().getTime();

				callback();
			});
		} else {
			callback();
		}
	};

	this.friendShipFunction = function (user) {
		if (this.userObject(user)) {
			return function () {
				user.friendShip(function (ok) {
					if (ok) {
						$("[name='friendShipDelete" + user.getUserID() + "']").hide();
						//$("name=firendShipText" + user.getUserID()).hide();
					}
				}, 1);
			};
		}
	};

	this.userObject = function (obj) {
		return obj instanceof User;
	};

	this.friends = function () {
		return friends;
	};

	this.friendsUser = function (callback) {
		this.loadUsers(friends, ssn.userManager.BASIC, function (u) {
			callback(u);
		});
	};

	this.friendShipRequestsUser = function (callback) {
		this.loadUsers(friendShipRequests, ssn.userManager.BASIC, function (u) {
			callback(u);
		});
	};

	this.friendShipRequests = function () {
		return friendShipRequests;
	};

	this.friendShipRequested = function () {
		return friendShipRequested;
	};

	this.getPublicKeys = umGetPublicKeys;
	this.search = umSearch;
	this.loadUsers = umLoadUsers;
	this.getUser = umGetUser;
	this.userLoaded = umUserLoaded;

	this.loadFriends = umLoadFriends;
};

var userManager = new ssn.userManager();
ssn.userManager = userManager;

ssn.userManager.BASIC = 1;
ssn.userManager.FULL = 10;